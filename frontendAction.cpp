...
struct CompileCommand {
    ...
    std::vector<std::string> includePaths;  // 解析后的绝对 -I 路径
};
        ...
        for (size_t i = 0; i < rawArgs.size(); ++i) {
            ...
            if (arg.size() >= 2 && arg[0] == '-' && arg[1] == 'I') {
                ...
                if (incPath.empty() && i + 1 < rawArgs.size())
                    incPath = rawArgs[++i];
                if (!incPath.empty()) {
                    if (!llvm::sys::path::is_absolute(incPath) && !CC.directory.empty()) {
                        SmallString<256> AbsPath(CC.directory);
                        llvm::sys::path::append(AbsPath, incPath);
                        incPath = std::string(AbsPath);
                    }
                    CC.includePaths.push_back(incPath);
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (!ProjectDir.empty()) {
        ...
        // 共享 Invocation 作为 fallback（显式指定系统头文件路径）
        ...
        for (const auto &f : projectFiles) {
            ...
            if (it != cdbMap.end() && !it->second.includePaths.empty()) {
                // 复制共享 Invocation（保留系统头文件路径），追加逐文件 -I
                PerFileInvocation = std::make_shared<CompilerInvocation>(*SharedInvocation);
                auto &HSOpts = PerFileInvocation->getHeaderSearchOpts();
                for (auto &p : it->second.includePaths)
                    HSOpts.AddPath(p, clang::frontend::Angled, false, false);
    ...
    // 一次 CreateFromArgs：共享系统头文件路径（显式指定系统头文件路径）
    std::vector<const char *> FakeArgs = {
        "frontendAction",
        "-fsyntax-only",
        "-isystem", "/usr/lib/llvm-18/lib/clang/18/include",
