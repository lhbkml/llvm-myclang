...
v4.0 开启全新的「代码行数统计」主题。在 AST 分析之外，对源文件进行文本级扫描，按代码行、空行、注释行分类计数，并区分单行注释 (`//`) 和多行注释 (`/* */`)。
...
在 `analyzeFile()` 中，AST 分析完成后逐行读取源文件，一次遍历同时完成：总行计数、长行检测（v3.1）、行数分类（v4.0）。
...
v4.0  行数统计     — 代码行/空行/注释行分类 + // vs /**/ 拆分
```

---

*本文档版本 4.0，归档于 2026-06-17。*
