...
static json::Value toJSON(const AnalysisResult &R) {
    ...
    if (auto *O = json.getAsObject()) {
        if (!R.errors.empty())
            (*O)["errors"] = json::Array(R.errors);
        if (!R.warnings.empty())
            (*O)["warnings"] = json::Array(R.warnings);
        (*O)["success"] = R.success;
    }
    return json;
}
...
static AnalysisResult runClangPipeline(CompilerInstance &CI,
    ...
    CI.getPreprocessor().addPPCallbacks(
        std::make_unique<IncludeCounter>(result.stats.includeCount));
...
static AnalysisResult analyzeSourceCode(const std::string &Code,
    ...
    // Clang 管线
    AnalysisResult result = runClangPipeline(CI, SharedInvocation, FileName, "");
...
static AnalysisResult analyzeFile(const std::string &FilePath,
    ...
    AnalysisResult result = runClangPipeline(CI, SharedInvocation, FilePath, ParentDir);
