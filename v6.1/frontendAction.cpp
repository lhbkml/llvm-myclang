...
static bool runClangPipeline(CompilerInstance &CI,
...
// ====================== 分析源码字符串（v6.1 — 程序化入口）======================
static AnalysisStats analyzeSourceCode(const std::string &Code,
                                        const std::string &FileName,
                                        const CompilerInvocation &SharedInvocation) {
    AnalysisStats Stats;
    ...
    CompilerInstance CI;
    CI.setVirtualFileSystem(MemFS);

    // Clang 管线（include 父目录用空——内存源码没有磁盘路径）
    runClangPipeline(CI, SharedInvocation, FileName, "", Stats);

    // 文本行分析
    classifyLines(Code, FileName, Stats);
...
static bool analyzeFile(const std::string &FilePath,
    ...
    CI.setVirtualFileSystem(RealFS);

    // 父目录用于 #include "" 解析
    std::string ParentDir = llvm::sys::path::parent_path(FilePath).str();

    if (!runClangPipeline(CI, SharedInvocation, FilePath, ParentDir, Stats))
        return false;
    ...
    if (src.is_open()) {
        ...
        classifyLines(code, FilePath, Stats);
    }
...
int main(int argc, char **argv) {
    ...
    if (ReadFromStdin) {
        ...
        AnalysisStats stats = analyzeSourceCode(code, "<stdin>", *Invocation);
        if (JsonOutput) {
            json::Value json = toJSON(stats);
            if (auto *O = json.getAsObject())
                (*O)["files"] = json::Array({"<stdin>"});
            outs() << formatv("{0:2}", json) << "\n";
        } else {
            printReport(stats, {"<stdin>"});
        }
        return 0;
    }
